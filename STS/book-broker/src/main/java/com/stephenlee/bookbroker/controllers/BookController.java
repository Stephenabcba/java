...
import com.stephenlee.bookbroker.services.BookService;
...
public class BookController {
	...
	public String brokerDashBoard(HttpSession session, Model model) {
		if (session.getAttribute("uuid") == null) {
			return "redirect:/";
		}
		List<Book> books = bookService.findAllUnborrowedBooks();
		List<Book> borrowedBooks = bookService.findAllBorrowedByUserId((Long) session.getAttribute("uuid"));
		model.addAttribute("books", books);
		model.addAttribute("borrowedBooks", borrowedBooks);
		return "brokerDashboard.jsp";
	}
    ...
    public String borrowBook(@PathVariable("id") Long id, HttpSession session) {
    	...
    	Book book = bookService.findBook(id);
    	if (book == null || book.getBorrower() != null || book.getPoster().getId().equals((Long) session.getAttribute("uuid")) ) {
        	return "redirect:/bookmarket";
        }
    	book.setBorrower(userService.findUser((Long) session.getAttribute("uuid")));
    ...
    public String returnBook(@PathVariable("id") Long id, HttpSession session) {
    	...
    	Book book = bookService.findBook(id);
    	if (book == null || book.getBorrower() == null || book.getPoster().getId().equals((Long) session.getAttribute("uuid")) 
    			|| !book.getBorrower().getId().equals((Long) session.getAttribute("uuid")) ) {
        	return "redirect:/bookmarket";
        }
